...
export default function Album() {
  ...
  const handleFilterClick = useCallback((f: FilterKey) => setActiveFilter(f), []);
  ...
  return (
    <section
      ref={sectionRef}
      /*
        pt-28 (112px) = fixed navbar pill height (~70px) + 8px top gap (pt-2)
        + extra breathing room. Adjust if your navbar pill height differs.
      */
      className="relative min-h-screen bg-black py-12 sm:py-16 lg:py-24 pt-28 sm:pt-32 px-4 sm:px-6 lg:px-8 overflow-hidden"
    >
      <div className="absolute inset-0 opacity-[0.015] pointer-events-none bg-[url('/noise.png')] bg-repeat" />
      ...
      <div className="relative max-w-7xl mx-auto z-10">
        ...
        <div className="album-header text-center mb-8 sm:mb-12 lg:mb-16 space-y-3 sm:space-y-4">
          ...
          <p className="text-sm sm:text-base lg:text-lg text-gray-400 max-w-2xl mx-auto px-4">
            A curated collection of moments captured through our lens — from
            campus life to creative explorations, each album tells its own story.
          </p>
        </div>
        ...
        {(searchQuery || activeFilter !== "all") && (
          <div className="text-center mb-8">
            <p className="text-sm text-gray-400">
              Showing{" "}
              <span className="text-white font-semibold">{filteredAlbums.length}</span>{" "}
              of{" "}
              <span className="text-white font-semibold">{ALBUMS.length}</span>{" "}
              albums
              {searchQuery && (
                <span className="ml-2">
                  for{" "}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mb-12">
          {cards.length > 0 ? (
            ...
            <div className="col-span-full text-center py-20">
              <div className="text-6xl mb-6">🔍</div>
              <h3 className="text-2xl text-white font-bold mb-3">No albums found</h3>
              <p className="text-base text-gray-400 mb-8 max-w-md mx-auto">
                Try adjusting your search or filter criteria to find what you&apos;re looking for.
              </p>
              <button
